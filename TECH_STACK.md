...
프로젝트 루트의 `.env.local` 파일에 들어갈 표준 환경 변수 설정입니다:
...
## 📦 통합 설치 원클릭 명령어 (Quick Install)

Next.js 16 기반 프로젝트 생성 후 실행할 수 있는 패키지 일괄 설치 스크립트입니다:

```bash
npm install @supabase/supabase-js @supabase/ssr ai @ai-sdk/google lucide-react clsx tailwind-merge framer-motion solapi
npm install -D tailwindcss @tailwindcss/postcss postcss typescript @types/node @types/react @types/react-dom
```
